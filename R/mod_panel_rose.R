...
mod_panel_rose_server <- function(id, adt, dat, fselection, sselection = NULL, ftitle = "title", stitle = "title", class = "topleft white",
                                  ...
                                  top = "98%", left = "1%", isRNA = FALSE, quant = c(1,6)){
    ...
    observeEvent(dataListen(),{
      if(is.null(adt)){
        ...
        if(is.null(sselection)){
          ...
          fexp <- adt[(rownames(adt) %in% rownames(dat)[fselection]), ]
          fncell <- nrow(fexp)
          fdata <- find_markers(extra = 0, n = 10, mat = fexp, quant = quant, zero = ifelse(isRNA, mean(colMeans(fexp)), 3))

          # Second selection
          sexp <- adt[(rownames(adt) %in% rownames(dat)[sselection]), ]
          sncell <- nrow(sexp)
          sdata <- find_markers(extra = 0, n = 10, mat = sexp, quant = quant, zero = ifelse(isRNA, mean(colMeans(fexp)), 3))
